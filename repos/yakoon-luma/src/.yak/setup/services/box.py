...
class BoxService:
    ...
    async def add_box(
        self, *, world_id: str, parent_id: str | None, name: str, description: str, portable: bool = False
    ) -> Box:
        ...
        data = BoxData(
            world_id=world_id, parent_id=parent_id, name=name, description=description, portable=portable
        )
        await self._on_replace(key=box_key(str(next_id)), doc=data.to_dict())
        return Box(
            id=str(next_id), world_id=world_id, parent_id=parent_id, name=name, description=description, portable=portable
        )
    ...
    async def get_box(self, box_id: str) -> Box | None:
        ...
        return Box(
            id=box_id, world_id=data.world_id, parent_id=data.parent_id, name=data.name, description=data.description, portable=data.portable
        )

    async def list_boxes(self, *, world_id: str, parent_id: str | None) -> list[Box]:
        return [b for b in await self._all_boxes() if b.world_id == world_id and b.parent_id == parent_id]
        ...
        if name.lower() != box.name.lower():
            for b in await self._all_boxes():
                if b.world_id == box.world_id and b.name.lower() == name.lower():
                    raise ValueError(f"Box '{name}' already exists in this world.")
        data = BoxData(world_id=box.world_id, parent_id=box.parent_id, name=name, description=description, portable=box.portable)
        await self._on_replace(key=box_key(box_id), doc=data.to_dict())
        return Box(id=box_id, world_id=box.world_id, parent_id=box.parent_id, name=name, description=description, portable=box.portable)
    ...
    async def move_box(self, box_id: str, new_parent_id: str | None) -> Box:
        ...
        data = BoxData(world_id=box.world_id, parent_id=new_parent_id, name=box.name, description=box.description, portable=box.portable)
        await self._on_replace(key=box_key(box_id), doc=data.to_dict())
        return Box(id=box_id, world_id=box.world_id, parent_id=new_parent_id, name=box.name, description=box.description, portable=box.portable)
