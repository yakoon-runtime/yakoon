...
class ExitService:
    ...
    async def _all_exits(self) -> list[Exit]:
        ...
        for r in rows:
            ...
            result.append(
                Exit(
                    id=r.key.id, world_id=data.world_id, source_box_id=data.source_box_id,
                    target_box_id=data.target_box_id, name=data.name, description=data.description, direction=data.direction
                )
            )
        return result
    ...
    async def get_exit(self, exit_id: str) -> Exit | None:
        ...
        return Exit(id=exit_id, world_id=data.world_id, source_box_id=data.source_box_id,
                    target_box_id=data.target_box_id, name=data.name, description=data.description, direction=data.direction)
    ...
    async def connect(
        self, *, world_id: str, source_box_id: str, target_box_id: str, name: str, description: str = "", direction: str = ""
    ) -> Exit:
        ...
        data = ExitData(world_id=world_id, source_box_id=source_box_id, target_box_id=target_box_id, name=name, description=description, direction=direction)
        await self._on_replace(key=exit_key(str(next_id)), doc=data.to_dict())
        return Exit(id=str(next_id), world_id=world_id, source_box_id=source_box_id, target_box_id=target_box_id, name=name, description=description, direction=direction)
