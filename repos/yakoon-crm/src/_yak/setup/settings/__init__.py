    ...
    def load(cls, config_dir: Path | None = None) -> Settings:
        ...
        return cls(
            storage=StorageSettings(
                backend=_backend(os.getenv("CRM_STORE_BACKEND") or storage_data.get("backend"), "memory"),
                dsn=os.getenv("CRM_STORE_DSN") or storage_data.get("dsn") or "",
            ),
            sequencer=SequenceSettings(
                backend=_backend(os.getenv("CRM_SEQUENCER_BACKEND") or sequencer_data.get("backend"), "memory"),
