...
class WorldService:
    ...
    async def add_world(self, *, name: str, description: str) -> World:
        if await self.get_world_by_name(name):
        ...
        await self._on_replace(key=world_key(str(next_id)), doc=data.to_dict())
    ...
    async def get_world_by_name(self, name: str) -> World | None:
        for w in await self.list_worlds():
    ...
    async def update_world(
        ...
        *,
        ...
        data = WorldData(
            name=new_name,
            description=description if description is not None else world.description,
            entry_box_id=(
                entry_box_id if entry_box_id is not None else world.entry_box_id
            ),
        )
