...
from ..ports import OnAppend, OnDelete, OnGet, OnReplace, OnScan
from .namespaces import world_key, world_namespace


class WorldService:

    def __init__(self, on_get: OnGet, on_append: OnAppend, on_replace: OnReplace, on_scan: OnScan, on_delete: OnDelete, on_next_id):
        self._on_get = on_get
    ...
    async def add_world(self, *, name: str, description: str) -> World:
        ...
        next_id = await self._on_next_id()
        key = world_key(str(next_id))
        data = WorldData(name=name, description=description)
        await self._on_append(key=key, value=data.to_dict())
    ...
    async def get_world(self, world_id: str) -> World | None:
        ...
        return World(id=world_id, name=data.name, description=data.description, entry_box_id=data.entry_box_id)
    ...
    async def update_world(self, *, world_id: str, name: str | None, description: str | None, entry_box_id: str | None = None) -> World:
        ...
        return World(id=world_id, name=new_name, description=new_desc, entry_box_id=new_entry)
    ...
    async def set_entry(self, *, world_id: str, box_id: str) -> World:
        return await self.update_world(world_id=world_id, name=None, description=None, entry_box_id=box_id)

    def _to_world(self, row) -> World:
        data = WorldData.from_dict(row.require_object())
        return World(id=row.key.id, name=data.name, description=data.description, entry_box_id=data.entry_box_id)
