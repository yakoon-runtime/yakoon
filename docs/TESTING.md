...
## 8. Note on Dead Imports (Web/SSH)

The Web and SSH apps currently crash on import. This is because the runtime was massively changed in recent days and the apps haven't been updated yet.

This is **not a structural problem**, but **3 minutes of work** — the imports need updating to the new package names. The architecture is designed for these apps to work; they just haven't been adapted yet.

---

## 9. 100% Test Coverage — Why It's Achievable

The architecture was designed from the start for **maximum testability**.

### Why this isn't empty promises

| Property | Testability |
|----------|-------------|
| **Ports instead of global services** | Every dependency is swappable in tests |
| **Immutable Projections** | Can be created and compared directly |
| **Generator flows** | Step-by-step testable without scheduler |
| **Pure compiler functions** | Input → output, no side effects |
| **Container instead of singletons** | Each test gets fresh containers |
| **Nodes are dataclasses** | No hidden initialization logic |
| **Permissions via ports** | Can be disabled/overridden in tests |

### Additionally: AI-generated tests

Because the architecture is clean (ports, pure functions, immutable model), an AI can derive tests directly from code structure:
...
No magic (no global state, no implicit dependencies, no constructors with side effects) means an AI can derive from `ports.get(OnX)` directly what needs to be mocked.

**100% test coverage is not a pious wish — it's a logical consequence of the architecture.**
